...
Spring Boot 4.0.6 환경과 Querydsl을 기반으로 구축한 고성능 의류 카테고리별 최저가/최고가 분석 및 검색 플랫폼 토이 프로젝트입니다.
대용량 트래픽 상황에서의 대형 조회 쿼리 병목 상황을 상정하고, 전용 통계 조회 테이블을 운용하여 효율적인 조회 시스템을 구축했습니다.
...
- **문제 인식**: 수천만 건의 실시간 상품 데이터(products)를 가진 이커머스 도메인에서 사용자가 카테고리별 최저가를 요청할 때마다 데이터베이스에 Aggregate(MIN(), GROUP BY) 집계 연산을 하게되면 디스크 I/O 병목으로 인해 전체 인프라가 다운될 위험이 있습니다.
...
- 단일 카테고리 내에서 최저가 상품 정보와 최고가 상품 정보를 동시에 뽑아 한 번에 리턴해야 하는 스펙을 만족하기 위해 ProductsRepositoryImpl에 Querydsl를 도입했습니다.
...
DELETE /api/admin/brand/{id} : 브랜드 정보 제거 및 하위 연관 상품 Cascade 정리(미구현)

POST /api/admin/product : 특정 브랜드 하위에 속하는 신규 의류 상품 추가 (동시 최저가 실시간 장부 동기화 트리거)
